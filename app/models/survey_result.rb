...
#### the rescue is not working!
  ...
  #############################################
  # summarize one column in this table using the weighted scale
  # - row: item that appears down the rows (left side)
  # - options:
  #   - filter: if provided, indicates a field and value to filter the data by
  #           format: {code: ____, value: ______}
  ...
  #   - data: series data
  # - map: data formatted for leaflet
  def self.weighted_onevar_count(row, options={})
    filter = options[:filter]
    exclude_dkra = options[:exclude_dkra] == true
    logger.debug "//////////// onevar vars - row: #{row}, filter: #{filter}, exclude_dkra: #{exclude_dkra}"
    result = {}

    # get the weighted col
    weight = SurveyQuestion.weighted

    if weight.blank?
      logger.debug "!!!!!!!!! weight does not exist, so calling non-weighted method"
      # no weight so call non-weighted method
      return onevar_count(row, options)
    end
    ...
    if q_row.present?
      ...
      begin
        ...
        data = call_sproc("call survey_weighted_onevar_count('#{weight.code}', '#{row}', '#{sql_filter}')")
...
#### the rescue is not working!
      ...
      if data.present?
        ...
        result[:row_answers].each do |row_answer|
          ...
          if data_row.present?
            # ok, found row
            result[:counts] << data_row['count'].round
          else
            # this answer had no values, so set to zero
            result[:counts] << 0
          end
...
#        result[:chart][:data] << {data: result[:counts]}
  ...
  #############################################
  # create a crosstab on two columns in this table using a weighted scale
  # - row: item that appears down the rows (left side)
  # - column: item that appears across the columns (top side)
  # - filter: if provided, indicates a field and value to filter the crosstab by
  #           format: {code: ____, value: ______}
  ...
  #   - data: series data
  # - map: data formatted for leaflet
  def self.weighted_crosstab_count(row, column, options={})
    filter = options[:filter]
    exclude_dkra = options[:exclude_dkra] == true
    logger.debug "//////////// crosstab vars - row: #{row}, col: #{column}, filter: #{filter}, exclude_dkra: #{exclude_dkra}"
    result = {}
    ...
    if weight.blank?
      logger.debug "!!!!!!!!! weight does not exist, so calling non-weighted method"
      # no weight so call non-weighted method
      return crosstab_count(row, col, options)
    end
    ...
    if q_row.present? && q_col.present?
      ...
      begin
        ...
        data = call_sproc("call survey_weighted_crosstab_count('#{weight.code}', '#{row}', '#{column}', '#{sql_filter}')")
